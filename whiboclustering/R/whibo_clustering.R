                             ...
                             max_iteration = 20, no_of_restarts = 1)
{
  ...
  params <- list("normalization_type" = normalization_type, "cluster_initialization_type" = cluster_initialization_type,
                     "assignment_type" = assignment_type, "recalculation_type" = recalculation_type)

  #DATA NORMALIZATION
  data <- wc_normalize(data = data, normalization_type = normalization_type)
...
predict.whibo_cluster <- function(model, data)
{
  ...
  {
    return(wc_assignment(data = eval(call(name = as.character(wc_norm_types$Method[tolower(wc_norm_types$Type) == tolower(model$params$normalization_type)]), data)), centroids = model$centroids, assignment_type = model$params$assignment_type))
  }
}
